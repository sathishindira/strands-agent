...
4. Usage Examples:
   ...
   agent = Agent(tools=[use_aws_sts])

   # List S3 buckets
   result = agent.tool.use_aws_sts(
       service_name="s3",
...
See the use_aws_sts function docstring for more details on parameters and usage.
"""
...
def get_boto3_client(
    ...
    If profile_name is not provided, it assumes a role named 'Strands-Agent-Assume-Role'
    using an account number collected from user input.
    ...
    # If no profile_name, assume role using account number

    role_name = "Strands-Agent-Assume-Role"
...
TOOL_SPEC = {
    "name": "use_aws_sts",
    "description": (
        "Make a boto3 client call with the specified account_id, service, operation, and parameters. "
...
def use_aws_sts(tool: ToolUse, **kwargs: Any) -> ToolResult:
    """
    ...
    Args:
        tool: The ToolUse object containing:
            ...
            - input: Dictionary containing:
                - account_id: AWS account ID to assume role in (e.g., '123456789012')
    ...
    client = get_boto3_client(service_name, region, account_id, profile_name)
